...
if (!empty($_GET['id'])) {
    $palest_id = $_REQUEST['id'];
}
if (!empty($_POST)) {
    $palest_id = $_POST['id'];

    $sql = "SELECT COUNT(*) AS total FROM evento where palestrante_palest_id = '$palest_id'";
    ...
    if ($row['total'] == 1) {
        $_SESSION['erro_ao_deletar_palestrante'] = true;
        header('Location: status.php');
        exit();
    } else {
        $sql = "DELETE FROM palestrante where palest_id = '$palest_id'";
        $conexao->query($sql);
...
<head>
    ...
    <title>Excluir Palestrantes</title>
                    ...
                    <div class="shadow p-3 mb-5 bg-white rounded">
                        <div class="container">
                            <div class="span10 offset1">
                                <div>
                                    <center>
                                        <h3>Excluir Palestrantes</h3>
                                </div>

                                <form class="form-horizontal" action="ExcluirPalestrantes.php" method="post">
                                    <input type="hidden" name="id" value="<?php echo $palest_id; ?>" />
                                    <center>
                                        <div class="alert alert-danger"> Deseja Realmente Excluir este palestrante ?
                                        </div>
                                        <div class="form actions">
                                            <button type="submit" class="btn btn-danger">Sim</button>
                                            <a href="GerenciarPalestrante.php" type="btn"
                                                class="btn btn-default">Não</a>
                                        </div>
                                </form>
                            </div>
                        </div>

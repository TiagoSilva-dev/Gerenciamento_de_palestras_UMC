            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="shadow p-3 mb-5 bg-white rounded">
                        <center>
                            <h1>Minha Lista de eventos</h1>
                            <hr>
                    </div>
                    <form action="MeusEventos.php" method="POST">
                    <div class="shadow p-3 mb-5 bg-white rounded">
                        ...
                        <table class="table table-sm">
                            <thead class="thead-dark">
                                <tr>
                                    ...
                                    <th scope="col">Remover Inscriçao</th>
                                    
                                </tr>
                            </thead>
                            <?php
                            $sql = "SELECT id_evento,date_format(`evento_data`,'%d/%m/%Y') as 'data_formatada',date_format(`evento_hora_ini`,'%H:%i')as'evento_hora_ini',date_format(`evento_hora_ter`,'%H:%i')as'evento_hora_ter', evento_nome,palest_nome
		from inscritos inner join evento
        on evento.id_evento = inscritos.evento_id_evento 
        inner join palestrante
		on palestrante.palest_id = evento.palestrante_palest_id
		where inscritos.participante_part_rgm_matricula = '$user' order by evento_data";

                            $resultado = mysqli_query($conexao, $sql);

                            while ($registro = mysqli_fetch_array($resultado)) {
                                echo '<td>' . $registro['evento_nome'] . '</td>';
                                echo "<td>" . $registro['palest_nome'] . "</td>";
                                echo "<td>" . $registro['data_formatada'] . "</td>";
                                echo "<td>" . $registro['evento_hora_ini'] . " AS " . $registro['evento_hora_ter'] . "</td>";
                                echo '<td><a href="ExcluirInscricao.php?idevento=' . $registro['id_evento'] . '"><span style="color: red;"><i class="fas fa-trash-alt" style=" font-size: 20px">Remover</i></a>';
                                echo '</td>';
                                echo "</tr>";
                            }
                            echo "</table>";
